...
## 課程思維導圖：數據埋點與增長圖譜
### 從數據採集到預測性經營
* **事件模型**：理解 GA4 的「一切皆事件」邏輯，區分自動、建議與自訂事件。
* **實作流程**：完成環境初始化，實作 \`logEvent\` 並正確註冊後台「自訂維度」。
* **驗證與分析**：利用 DebugView 即時除錯，透過漏斗分析與受眾分群定位增長點。
* **AI 賦能**：應用預測性指標（流失/購買機率），精準分配廣告預算。
* **合規與大數據**：實作同意模式 v2 保護隱私，匯出 BigQuery 拿回數據主權。
