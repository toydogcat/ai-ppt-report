...
## 課程思維導圖：AI 搜尋優化全景
### 從「被點擊」轉向「被引用與推薦」
* **技術基礎**：傳統 Technical SEO 依然重要，是爬蟲理解網站結構的地基。
* **引用優化 (GEO)**：針對 Perplexity/ChatGPT Search 的引用演算法，強化數據表與直接解答。
* **心智佔領 (AIEO)**：深入模型預訓練語料庫，讓品牌成為大模型的「常識」與「權威方案」。
* **外部信任**：建立 Third-Party Trust，利用論壇與公關產出 AI 信任的第三方背書。
* **指標轉化**：從點擊率 (CTR) 轉向模型佔有率 (Share of Model)，重新定義轉換路徑。
