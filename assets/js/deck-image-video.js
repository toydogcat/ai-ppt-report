...
## 課程思維導圖：生圖生影片 AI 創作流
### 從「抽卡碰運氣」走向「導演級精準控制」
* **提示詞工程**：掌握結構化 Prompt 公式，精確控制光影、構圖與相機參數。
* **一致性控制**：熟練 \`--cref\` (角色) 與 \`--sref\` (風格) 鎖定，確保分鏡不崩壞。
* **動態生成**：學習電影級運鏡參數 (Pan/Tilt/Zoom)，利用 Veo 3.1 模擬真實物理。
* **工業工作流**：建立「以圖生影」的錨定技術，徹底解決影片閃爍與變形問題。
* **進階控制**：應用 ControlNet (骨架/深度) 與局部重繪，實現像素級的微調。
